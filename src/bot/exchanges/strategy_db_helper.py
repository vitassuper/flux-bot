...
from src.db.models import Order, Deal
from src.bot.objects.deal_stats import DealStats
from src.db.services.deal import (
    update_deal,
    create_deal,
    increment_safety_orders_count,
    get_or_create_deal,
    get_deal_or_fail,
)
from src.db.services.order import create_order, get_orders_volume, get_deal_stats
...
class StrategyDBHelper:
    def __init__(
        self, side: SideType, bot_id: int, pair: str, position: Union[int, None] = None
    ):
        self.side = side
        self.bot_id = bot_id
        self.pair = pair
    ...
    async def create_open_order(
        self, deal_id: int, price: Decimal, volume: Decimal
    ) -> Order:
        return await create_order(
            deal_id=deal_id,
            side=self.get_order_side(action="open"),
            price=price,
            volume=volume,
        )

    async def create_average_order(
        self, deal_id: int, price: Decimal, volume: Decimal
    ) -> Order:
        return await create_order(
            deal_id=deal_id,
    ...
    async def create_close_order(
        ...
        return await create_order(
            deal_id=deal_id,
            side=self.get_order_side(action="close"),
            price=price,
            volume=volume,
        )
    ...
    async def get_deal(self) -> Deal:
        return await get_deal_or_fail(
            bot_id=self.bot_id, pair=self.pair, position=self.position
        )
