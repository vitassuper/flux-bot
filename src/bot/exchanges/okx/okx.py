...
from src.bot.exchanges.base import BaseExchange
...
class Okex(BaseExchange):
    ...
    def get_current_leverage(
        ...
        leverage_info = next(
            (
                l
                for l in self.ccxt_exchange.fetch_leverage(
                    pair, {"mgnMode": margin_type.value}
                )["data"]
                if l["posSide"] == side
            ),
            None,
    ...
    def get_opened_long_position(self, pair: str) -> Union[Dict, None]:
        ...
        return next(
            (
                p
                for p in positions
                if p["symbol"] == pair and p["side"] == SideType.long
            ),
            None,
        )
    ...
    def get_opened_short_position(self, pair: str) -> Union[Dict, None]:
        ...
        return next(
            (
                p
                for p in positions
                if p["symbol"] == pair and p["side"] == SideType.short
            ),
            None,
        )
    ...
    def add_margin_to_short_position(self, pair: str, amount: float):
        self.ccxt_exchange.add_margin(
            symbol=pair, amount=amount, params={"posSide": SideType.short.value}
        )

    def add_margin_to_long_position(self, pair: str, amount: float):
        self.ccxt_exchange.add_margin(
            symbol=pair, amount=amount, params={"posSide": SideType.long.value}
        )
    ...
    def set_leverage(
        ...
        current_leverage = self.get_current_leverage(
            pair=pair, side=side, margin_type=margin_type
        )
    ...
    def set_leverage_for_short_position(
        ...
        self.set_leverage(leverage=leverage, pair=pair, margin_type=margin_type)
    ...
    def set_leverage_for_long_position(
        ...
        self.set_leverage(
            leverage=leverage, pair=pair, side=SideType.long, margin_type=margin_type
        )
