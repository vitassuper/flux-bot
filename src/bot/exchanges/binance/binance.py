...
class Binance(BaseExchange):
        ...
        exchange = ccxt.binance(
            {
                ...
                "options": {"defaultType": "future", "hedgeMode": hedge_mode},
    ...
    def get_current_leverage(
        self, pair: str, side: Union[SideType.long, SideType.short] = SideType.short
    ):
        ...
        position = next(
            (p for p in positions if p["info"]["positionSide"] == position_side), None
        )
    ...
    def get_opened_short_position(self, pair: str) -> Union[Dict, None]:
        ...
        return next(
            (p for p in positions if p["contracts"] and p["side"] == SideType.short),
            None,
        )
    ...
    def get_opened_long_position(self, pair: str) -> Union[Dict, None]:
        ...
        return next(
            (p for p in positions if p["contracts"] and p["side"] == SideType.long),
            None,
        )
    ...
    def buy_short_position(
        ...
        return self.ccxt_exchange.create_market_buy_order(
            symbol=pair, amount=amount, params=params
        )
