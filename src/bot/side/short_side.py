...
class ShortSide(BaseSide):
    def ensure_deal_not_opened(self, pair: str) -> None:
        ...
        if open_position:
            raise ConnectorException(
                f"the specified position already exists on the exchange: {pair}"
            )
    ...
    def ensure_deal_opened(self, pair: str) -> None:
        ...
        if not open_position:
            raise ConnectorException(
                f"the specified position does not exist on the exchange: {pair}"
            )
    ...
    def add_margin(self, pair: str, quote_amount: float):
        if self.margin_type == MarginType.cross:
            raise ConnectorException("Cant add margin to cross-margined position")

        self.exchange.add_margin_to_short_position(pair=pair, amount=quote_amount)
