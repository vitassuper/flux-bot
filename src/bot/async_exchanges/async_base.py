...
from src.db.services.deal import get_opened_deals
    ...
    async def get_open_positions_info(self) -> List[ActiveDealMessage]:
        ...
        async with self as exchange:
            ...
            for item in exchange_positions:
                ...
                deal = next((x for x in deals if x.pair == symbol), None)

                liquidation_price = (
                    self.exchange.price_to_precision(symbol, item["liquidationPrice"])
