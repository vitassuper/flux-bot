...
from src.bot.side.base_side import BaseSide
from src.bot.exchanges.strategy_db_helper import StrategyDBHelper
...
from src.db.services.deal import get_all_grid_deals
...
class BaseStrategy(metaclass=abc.ABCMeta):
    def __init__(
        ...
        self.active_deal_model = deal

        self.contract_size = Decimal(
            self.exchange.get_market(pair=pair)["contractSize"]
        )

        self.strategy_helper = StrategyHelper(
            taker_fee=Decimal(self.exchange.ccxt_exchange.market(self.pair)["taker"]),
            side=side.get_side_type(),
            contract_size=self.contract_size,
        )

        self.db_helper = StrategyDBHelper(
            side=side.get_side_type(),
            bot_id=self.bot_id,
            pair=self.pair,
    ...
    async def open_deal(self, amount: float) -> OpenedDealMessage:
        ...
        if deal.position:
            positions_string = ",".join(
                [str(deal.position) for deal in await get_all_grid_deals(deal)]
            )
        ...
        return OpenedDealMessage(
            ...
            deal_id=deal.id,
            quote_amount=self.exchange.ccxt_exchange.cost_to_precision(
                self.pair, quote_amount
            ),
            price=price,
    ...
    async def average_deal(
        ...
        base_amount = self.get_base_amount(quote_amount=Decimal(amount))
        safety_count, quote_amount = await self.average_deal_process(
            base_amount=base_amount
        )

        deal_stats = await self.db_helper.get_deal_stats(
        ...
        return AveragedDealMessage(
            ...
            price="0",
            side=self.side.get_side_type(),
            total_quote_amount=self.exchange.ccxt_exchange.cost_to_precision(
                self.pair, deal_stats.total_quote_amount * self.contract_size
            ),
        )
    ...
    async def close_deal(self, amount: float) -> ClosedDealMessage:
        ...
        if closed_deal.deal.position:
            positions_string = ",".join(
                [
                    str(deal.position)
                    for deal in await get_all_grid_deals(closed_deal.deal)
                ]
            )
        ...
        return ClosedDealMessage(
            ...
            base_amount=0.1,  # TODO: remove magic number,
            quote_amount=self.exchange.ccxt_exchange.cost_to_precision(
                self.pair, closed_deal.quote_amount
            ),
            safety_orders_count=closed_deal.deal.safety_order_count,
            duration=StrategyHelper.get_time_duration_string(
                date_open=closed_deal.deal.date_open, date_close=datetime.now()
            ),
            profit=self.exchange.ccxt_exchange.decimal_to_precision(
                closed_deal.deal.pnl, TRUNCATE, 4
            ),
    ...
    def get_base_amount(self, quote_amount: Decimal) -> Decimal:
        ...
        return Decimal(
            self.side.exchange.get_base_amount(
                pair=self.pair, quote_amount=quote_amount
            )
        )
