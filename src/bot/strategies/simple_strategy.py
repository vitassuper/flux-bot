...
class SimpleStrategy(BaseStrategy):
    async def open_deal_process(self, amount: float):
        ...
        await self.db_helper.create_open_order(
            deal_id=deal.id, price=price, volume=order.volume
        )
    ...
    async def average_deal_process(self, amount: float):
        ...
        # TODO: only for okex, should be refactored
        if self.bot_id == 1 or self.bot_id == 3:
            self.side.add_margin(
                pair=self.pair, quote_amount=float(quote_amount * Decimal(0.06))
            )

        return safety_count, quote_amount

    async def close_deal_process(
        self, amount: float = None, deal: Union[Deal, None] = None
    ) -> ClosedDeal:
