...
class GridStrategy(BaseStrategy):
    ...
    async def average_deal_process(self, base_amount: Decimal):
        ...
        await self.db_helper.create_average_order(deal_id=self.active_deal_model.id, price=order.price,
                                                  volume=order.volume)

        safety_count = await self.db_helper.average_deal(deal_id=self.active_deal_model.id)
    ...
    async def close_deal_process(self, amount: float = None) -> ClosedDeal:
        ...
        await self.db_helper.create_close_order(deal_id=self.active_deal_model.id, price=order.price,
                                                volume=order.volume)
        ...
        deal = await self.db_helper.close_deal(deal_id=self.active_deal_model.id, pnl=pnl)
