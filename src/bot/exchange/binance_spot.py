...
class BinanceSpot(SpotBaseExchange):
    ...
    def market_sell(self, pair: str, amount: int):
        return self.exchange.create_order(symbol=pair, type='market', side='sell', amount=amount)

    def market_buy(self, pair: str, amount: int):
        return self.exchange.create_order(symbol=pair, type='market', side='buy', amount=amount)
